...
useHead({
  ...
  link: [{ rel: 'canonical', href: 'https://briko.app/mosaic' }],
})
...
const isWorking = computed(
  () =>
    !!loading.value ||
    mosaic.status === 'working' ||
    mosaic.status === 'tiling' ||
    !!publishing.value
)
...
const formattedCost = computed(() => {
  try {
    return '$' + (mosaic.tilingResult?.estTotalCost ?? 0).toFixed(2)
  } catch {
    return '$0.00'
  }
})
...
function applyPreset(v: 'auto' | 'line' | 'pop') {
  ...
  try {
    if (srcBitmap.value) scheduleRegen()
  } catch {}
}
...
function nearestOption(val: number, opts: number[]) {
  let best = opts[0],
    bestD = Math.abs(opts[0] - val)
  for (const o of opts) {
    const d = Math.abs(o - val)
    if (d < bestD) {
      best = o
      bestD = d
    }
...
const widthSelStuds = computed<number>({
  get() {
    return nearestOption(target.value.w, dimOptions)
  },
  set(v: number) {
    const s = clamp(v, 16, 256)
    target.value.w = s
...
const heightSelStuds = computed<number>({
  get() {
    return nearestOption(target.value.h, dimOptions)
  },
  set(v: number) {
    const s = clamp(v, 16, 256)
    target.value.h = s
...
const mosaicTask = createWorkerTask<WorkerOut>(() =>
  import('@/workers/mosaic.worker?worker').then((m: any) => new m.default())
)
...
watch(
  ...
  (val: readonly string[]) => {
    ...
    if (didMount.value && typeof window !== 'undefined') {
      try {
        showToast(copy.mosaic.toasts.regenerating, 'info', 1500)
      } catch {}
    }
  },
  { immediate: true }
...
async function onFile(file: File) {
  ...
  try {
    try {
      showToast(copy.mosaic.toasts.generating, 'info', 1500)
    } catch {}
    ...
    try {
      ...
      fr.onload = () => {
        ;(window as any).__brikoOriginalDataUrl = fr.result as string
      }
      fr.readAsDataURL(file)
    } catch {}
    // Kick off fast Briko’d preview (WebP) using lightweight pipeline (bytes → worker)
    try {
      const { blob, mode: resolved } = await mosaicifyFromFile(file, {
        w: target.value.w,
        h: target.value.h,
        paletteId: 'briko-v1',
        mode: mode.value,
      ...
      try {
        showToast(
          `Auto: ${resolvedMode.value} • ${target.value.w}×${target.value.h} studs`,
          'success',
          1600
        )
      } catch {}
    } catch (e) {
      console.warn('[mosaicify] preview failed', e)
    }
    ...
    const {
      ...
      width: w64,
      height: h64,
    } = await imageBitmapToBuffer(srcBitmap.value!, previewQuality.value, previewQuality.value)
    const thumb = await mosaicTask.run(
      {
        ...
        palette: legoPalette,
        greedy: false,
        dither: useDither.value,
      },
      {
        onProgress: (p) => {
          if (typeof p?.pct === 'number') progress.value = p.pct
        },
        timeoutMs: 30000,
        transfer: [buf64],
      }
    ...
    const {
      ...
      width: w0,
      height: h0,
    } = await imageBitmapToBuffer(srcBitmap.value!, target.value.w, target.value.h)
...
async function onGenerate() {
  ...
  try {
    try {
      showToast(copy.mosaic.toasts.generating, 'info', 1200)
    } catch {}
    await mosaic.runGreedyTiling()
  } finally {
    mosaic.clearUiWorking()
...
function snapDim(which: 'w' | 'h') {
  ...
  if (cur !== nearest) {
    ...
    mosaic.setTargetSize(target.value.w, target.value.h)
    try {
      showToast('Snapped to nearest supported size', 'info', 1200)
    } catch {}
  } else {
...
function onDownloadCsv() {
  ...
  const rows = mosaic.tilingResult.bom.map((r: SimpleBomRow) => ({
    part: r.part,
    colorId: r.colorId,
    qty: r.qty,
  }))
...
function onDownloadBrickLink() {
  ...
  try {
    ...
    const rows = mosaic.tilingResult.bom.map((r: any) => {
      const partKey =
        (mosaic.settings.topSurface || 'plates') === 'tiles' ? `tile-${r.part}` : `plate-${r.part}`
      const colorName = legoPalette[r.colorId]?.name
      if (!colorName) {
        throw new Error(`Unknown color id: ${r.colorId}`)
      }
      return { partKey, colorKey: colorName, qty: r.qty }
    })
    const remarks = blAddRemarks.value
      ? `Briko • Mosaic ${mosaic.width}×${mosaic.height}`
      : undefined
    const xml = generateBrickLinkWantedXML(rows, { condition: blCondition.value, remarks })
    downloadWantedXml(
      xml,
      `briko-wanted-list-${mosaic.settings.topSurface || 'plates'}-${Date.now()}.xml`
    )
    try {
      showToast('Your BrickLink XML is ready!', 'success', 2000)
    } catch {}
  } catch (e: any) {
...
function onBuyClickHeader() {
  try {
    ;(window as any)?.gtag?.('event', 'click_buy_parts', { location: 'parts_list_header' })
  } catch {}
}
function onBuyClickSticky() {
  try {
    ;(window as any)?.gtag?.('event', 'click_buy_parts', { location: 'parts_list_sticky_mobile' })
  } catch {}
}
...
watch(steps, () => {
  stepIdx.value = steps.value.length ? 1 : 0
})
...
function scheduleRegen() {
  ...
  if (mosaic.status === 'working' || mosaic.status === 'tiling') {
    regenQueued = true
    return
  }
  ...
  regenTimer = setTimeout(async () => {
    try {
      ...
      mosaic.setTargetSize(full.width, full.height)
      mosaic.setGrid(
        (full.quantizedIndexes || full.indexes) as Uint16Array,
        full.width,
        full.height
      )
      ...
      progress.value = 0
      if (regenQueued) {
        regenQueued = false // flush queued change
...
function scheduleRetile() {
  ...
  retileTimer = setTimeout(async () => {
    ...
    try {
      ...
      if (retileQueued) {
        retileQueued = false
        requestAnimationFrame(() => scheduleRetile())
      }
    }
  }, 200)
}
...
<template>
  <main class="mx-auto max-w-7xl px-6 py-10 text-[#343434] mb-20">
    ...
    <nav aria-label="Quick guide" class="mt-2 flex flex-wrap gap-6 md:gap-8 items-center">
      <a
        ...
        :class="[
          ...
          i <= activeStepIndex
            ? 'bg-[#FF0062] text-[#FFD808] shadow-md'
            : 'bg-[#2F3061] text-white',
        ]"
      >
        <span
        ...
        <span class="text-sm" :class="i <= activeStepIndex ? 'font-semibold' : ''">{{
          s.title
        }}</span>
      </a>
    </nav>
    ...
    <div class="mt-12 grid gap-8 lg:gap-12 lg:grid-cols-[460px,1fr] items-start">
        ...
        <aside class="space-y-6">
          ...
          <div class="sticky-parent relative">
            <div class="lg:sticky lg:top-24 z-0" aria-label="Mosaic parameters">
              ...
              >
                <div class="z-0 rounded-2xl border border-black/80 bg-[#FFD808] p-3 sm:p-4 space-y-6">
                  <!-- Step 1: Upload -->
                    ...
                    <StepCard :step="1" title="Upload your photo">
                      <template #actions>
                        ...
                        <div class="hidden md:flex items-center gap-2">
                          <button
                            type="button"
                            :class="chipCls(currentPreset === 'auto')"
                            @click="currentPreset = 'auto'"
                            :aria-pressed="currentPreset === 'auto'"
                          >
                            Auto
                          </button>
                          <button
                            type="button"
                            :class="chipCls(currentPreset === 'line')"
                            @click="currentPreset = 'line'"
                            :aria-pressed="currentPreset === 'line'"
                          >
                            Line Art
                          </button>
                          <button
                            type="button"
                            :class="chipCls(currentPreset === 'pop')"
                            @click="currentPreset = 'pop'"
                            :aria-pressed="currentPreset === 'pop'"
                          >
                            Photo Pop
                          </button>
                        </div>
                  ...
                  <section :id="stepsGuide[1].id" class="scroll-mt-28 pt-6">
                    <div class="flex items-center gap-3 mb-1">
                      ...
                      <div
                        class="inline-grid h-9 w-9 place-items-center rounded-xl border border-white/30 bg-white/70"
                      >
                        <span
                          class="material-symbols-rounded text-[20px] text-[#FF0062]"
                          aria-hidden="true"
                          >tune</span
                        >
                      </div>
                  ...
                  <div>
                    ...
                    <p class="mt-1 text-xs text-[#343434]/60">
                      Pick exact dimensions, or use the sliders above.
                    </p>
                    <div class="mt-2 grid grid-cols-2 sm:grid-cols-3 gap-2">
                      <div>
                        <label class="block text-xs text-[#2F3061] mb-1" for="sel-width"
                          >Width</label
                        >
                        <select
                          id="sel-width"
                          v-model.number="widthSelStuds"
                          class="w-full rounded-lg border border-[#343434]/20 bg-white text-[#2F3061] px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#FF0062]"
                          :aria-describedby="'desc-width'"
                        >
                          <option v-for="n in dimOptions" :key="'w' + n" :value="n">
                            {{
                              units === 'studs'
                                ? n
                                : units === 'inches'
                        ...
                        <span id="desc-width" class="sr-only"
                          >Exact mosaic width. Matches the sliders above.</span
                        >
                      </div>
                      <div>
                        <label class="block text-xs text-[#2F3061] mb-1" for="sel-height"
                          >Height</label
                        >
                        <select
                          id="sel-height"
                          v-model.number="heightSelStuds"
                          class="w-full rounded-lg border border-[#343434]/20 bg-white text-[#2F3061] px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#FF0062]"
                          :aria-describedby="'desc-height'"
                        >
                          <option v-for="n in dimOptions" :key="'h' + n" :value="n">
                            {{
                              units === 'studs'
                                ? n
                                : units === 'inches'
                        ...
                        <span id="desc-height" class="sr-only"
                          >Exact mosaic height. Matches the sliders above.</span
                        >
                      </div>
                      <div>
                        <label class="block text-xs text-[#2F3061] mb-1" for="sel-units"
                          >Units</label
                        >
                        <select
                          id="sel-units"
                        ...
                        <span id="desc-units" class="sr-only"
                          >Change display units only; brick count stays the same.</span
                        >
                      </div>
                    </div>
                  </div>
                  ...
                  <div class="mt-3" v-if="showAdvanced">
                    <label class="block text-sm" title="Pieces you want to build with"
                      >Allowed parts</label
                    >
                    <div class="grid grid-cols-3 gap-2 text-sm">
                      <label v-for="p in ALL_PARTS" :key="p" class="inline-flex items-center gap-2">
                        <input type="checkbox" :value="p" v-model="selectedParts" />
                        {{ p.replace('x', '×') }}
                      </label>
                    </div>
                  </div>
                  ...
                  <div class="mt-3" v-if="showAdvanced">
                    <label class="block text-sm text-[#2F3061]" for="sel-orientation"
                      >Orientation</label
                    >
                    <div class="flex items-center mt-1">
                      <div class="min-w-[160px] max-w-[220px]">
                        <select
                          id="sel-orientation"
                          v-model="mosaic.settings.snapOrientation"
                          class="w-full rounded-lg border border-[#343434]/20 bg-white text-[#2F3061] px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#FF0062]"
                        >
          ...
          <div v-if="mosaic.tilingResult" class="relative z-10 card-ivory p-5 rounded-2xl">
            ...
            <div class="flex flex-wrap gap-2">
              <button
                type="button"
                class="btn-purple-outline focus-cyber"
            ...
            <div class="relative -mx-4 mt-3 px-4">
              ...
              <ul class="divide-y divide-[color:var(--ivory-border)] text-sm">
                <li
                  v-for="row in mosaic.tilingResult.bom"
                  :key="row.part + '-' + row.colorId"
                  :id="'bom-' + row.part + '-' + row.colorId"
                  class="grid grid-cols-[1fr_auto] items-center py-2"
                >
                  <div class="flex items-center gap-2">
                    <span
                      class="h-3 w-3 rounded-sm ring-1 ring-[color:var(--ivory-border)]"
                      :style="{ background: legoPalette[row.colorId]?.hex || '#ccc' }"
                    ></span>
                    <span class="text-[var(--dark)]"
                      >{{ legoPalette[row.colorId]?.name || 'Color ' + row.colorId }} ·
                      {{ mosaic.settings.topSurface === 'tiles' ? 'Tile' : 'Plate' }}
                      {{ row.part.replace('x', '×') }}</span
                    >
                  </div>
                  <span class="tabular-nums text-sm font-semibold text-[color:var(--dark)/.70]"
                    >{{ row.qty }} pcs</span
                  >
                </li>
              </ul>
              ...
              <div
                class="sticky bottom-0 bg-gradient-to-t from-[#0B0F16]/90 to-transparent pt-3 pb-3 md:hidden"
              >
                <div class="flex justify-end">
                  <a
                    ...
                    target="_blank"
                    rel="noopener"
                    class="btn-pink focus-cyber h-11 px-4 inline-flex items-center gap-2"
                    aria-label="Where to buy pieces"
      ...
      <Transition
        appear
        enter-active-class="transition ease-out duration-700 delay-150"
        enter-from-class="opacity-0 translate-y-2"
        enter-to-class="opacity-100 translate-y-0"
      >
            ...
            >
              ...
              <div class="flex flex-wrap items-center gap-3 border-b border-white/10 px-5 py-3">
                ...
                <div class="ml-2 flex items-center gap-2 text-sm">
                  <button
                    :class="[
                      ...
                      tab === '2D' ? 'text-white border-b-2 border-[#FF0062]' : 'text-white/70',
                    ]"
                    @click="tab = '2D'"
                  >
                    2D Mosaic
                  ...
                  <button
                    :class="[
                      'px-3 py-1 rounded-md',
                      tab === '3D' ? 'text-white border-b-2 border-[#FF0062]' : 'text-white/70',
                    ]"
                    @click="tab = '3D'"
                  >
                    3D Preview
                ...
                <div class="ml-auto flex items-center gap-3">
                  <label
                    v-if="tab === '2D'"
                    class="inline-flex items-center gap-2 text-xs text-white/80"
                    :title="copy.mosaic.controls.showPlateOutlinesHelp"
                  >
                    <input type="checkbox" v-model="showPlates" />
                    <span>{{ copy.mosaic.controls.showPlateOutlines }}</span>
                  </label>
                  <div
                    v-if="mosaic.status === 'error'"
                    class="text-xs text-red-300 bg-red-500/10 px-3 py-1.5 rounded-full"
                  >
              ...
              >
                ...
                <div
                  v-if="dropActive"
                  class="absolute inset-0 rounded-2xl ring-2 ring-white/40 bg-white/5 pointer-events-none"
                ></div>

                <!-- Always-present preview window -->
                <div
                  class="min-h-[560px] rounded-xl bg-zinc-900/30 ring-1 ring-white/10 overflow-hidden grid place-items-center"
                >
                  <!-- Loading state -->
                        ...
                        <template v-else-if="cfg?.features?.builder3d">
                          <ClientOnly>
                            <div class="animate-fade-in-up">
                              <VoxelViewer
                                :bricks="mosaic.tilingResult?.bricks || []"
                                :visibleLayers="mosaic.visibleLayers"
                                :studSize="1"
                                :surface="(mosaic.settings.topSurface || 'plates') as any"
                              />
                            </div>
                          ...
                          <div class="mt-3">
                            <LayerSlider
                              :maxLayers="mosaic.height || 1"
                              :visibleLayers="mosaic.visibleLayers"
                              @update:visibleLayers="mosaic.setVisibleLayers"
                              :title="copy.builder3d.controls.layerSliderHelp"
                            />
                          </div>
                        </template>
                      </div>
                    ...
                    <footer class="mt-4 border-t border-white/10 relative">
                      ...
                      <div class="px-4 pt-4 pb-3 flex flex-wrap items-center justify-center gap-3">
                        <ButtonPrimary
                          type="button"
                          variant="pink"
                          class="inline-flex items-center gap-2 h-11 px-4 rounded-xl focus-cyber"
                          :disabled="!mosaic.isReady"
                          :title="!mosaic.isReady ? 'Preparing…' : ''"
                          @click="onDownloadPdf"
                        >
                          <!-- Download icon -->
                          <svg
                        ...
                        <button
                          class="btn-ivory-outline focus-cyber inline-flex items-center gap-2 h-11 px-4 rounded-xl"
                          :disabled="!mosaic.canExport"
                          :title="!mosaic.canExport ? 'Generate a mosaic to enable' : ''"
                          @click="onDownloadPng"
                        >
                        ...
                        <button
                          ...
                          :disabled="!mosaic.canExport"
                          @click="openBLDialog"
                          :title="'Downloads a BrickLink Wanted List you can upload at BrickLink → Wanted → Upload.'"
                        >
                          <!-- Package icon -->
                          <svg
                            viewBox="0 0 24 24"
                      ...
                      <div
                        v-if="showBL"
                        class="absolute right-4 z-20 mt-2 w-80 rounded-2xl border border-white/10 bg-black/70 p-4 shadow-soft-card"
                      >
                        <div class="flex items-center justify-between mb-2">
                          <div class="font-medium">BrickLink Export</div>
                          <button class="text-white/60 hover:text-white" @click="closeBLDialog">
                            ✕
                          </button>
                        </div>
                        <div class="space-y-3 text-sm">
                          <div>
                            ...
                            <label class="mr-3 inline-flex items-center gap-2"
                              ><input type="radio" value="N" v-model="blCondition" /> New</label
                            >
                            <label class="inline-flex items-center gap-2"
                              ><input type="radio" value="U" v-model="blCondition" /> Used</label
                            >
                          </div>
                          <label class="inline-flex items-center gap-2"
                            ><input type="checkbox" v-model="blAddRemarks" /> Add remarks “Briko •
                            Mosaic”</label
                          >
                          <div class="flex justify-end gap-2 pt-2">
                            <button
                              class="btn-ivory-outline focus-cyber px-3 py-1.5"
                              @click="closeBLDialog"
                            >
                              Cancel
                            </button>
                      ...
                      <div class="px-4 pb-2 flex items-center gap-4">
                        <p v-if="mosaic.tilingResult" class="text-white/80 text-sm">
                          ...
                          <span class="tabular-nums">{{ mosaic.tilingResult.bricks.length }}</span>
                          <span class="mx-3 opacity-30">•</span>
                          <span class="font-semibold">Est. cost:</span>
                          <span class="tabular-nums">{{ formattedCost }}</span>
                        </p>
                      </div>
                  ...
                  <template v-else-if="sourceImgUrl">
                    <img
                      :src="sourceImgUrl"
                      alt=""
                      role="presentation"
